...
int main() {
  ...
  // this actually has a small memory leak, because we should store the path in the heap in order to return the pointer to the string safely
  // this program is only small and we're only adding about 1000 bytes each time but probably to be good we should do something like:
  // char *path = find_path(...);
  // cout << path << endl << endl;
  // delete [] path
  // rather than just print straight out the anonymous pointer to the path string
